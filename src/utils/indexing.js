...
const getFileIndex = (indexFd, relativeOffset)=>{
    ...
    let low = 0;
    let high = NUM_ENTRIES-1; //buffer.size gives bytes, you absolute spongehead :)
    ...
    while(low<=high){
        ...
        if(key==relativeOffset) return buffer.readUInt32BE(offsetMid + 8);
        else if (key < relativeOffset) {
            best = mid; 
            low = mid + 1;
        } 
        else {
